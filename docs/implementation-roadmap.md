...
2. **NPU 证据层还偏浅**：已有五表框架，并已补 pipeline / compute /
   capacity skill 骨架；后续还需要 analyzer、schema 校验和真实 trace 验证。
...
- 五表输出契约。
- `skills/xllm-npu-pipeline-analysis/` 基础 skill 骨架。
...
当前 NPU 仓库已补 `skills/xllm-npu-compute-simulation/` 骨架和硬件/模型
索引占位，但还没有从模型 config 推导 operator flow、FLOPs、MFU、
kernel-to-op mapping 的完整 analyzer。
...
- 完善 `references/npu-specs.json`：910B3/A3 的 bf16/fp16/int8/fp8 峰值、带宽、UB/L1/L2、AICore 数量。
- 完善 `references/model-config-index.json`：Qwen3/Qwen3.5/DeepSeek/GLM/Kimi 等 NPU serving 常用模型。
...
当前仓库已补 `skills/xllm-npu-capacity-planner/` 骨架，用于承载 MTP
reserved linear bytes、KV blocks、HBM budget 和 OOM 风险解释。
