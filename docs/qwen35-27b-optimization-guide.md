...
```bash
export RUN_ROOT=<run-root>/$(date +%Y%m%d)_qwen35_27b_npu_sota
...
**实测数据集**: `workload_20k.jsonl` (20k 真实京东对话请求，平均 input ~19860 tokens)。
...
```bash
# 实测启动命令 (Phy 8, NPU 14)
<workspace>/xllm/build/xllm/core/server/xllm \
  --model <model-root>/Qwen35-27B \
...
**关键参数**:
- `--draft_model <model-root>/Qwen35-27B-mtp` — MTP draft 权重
