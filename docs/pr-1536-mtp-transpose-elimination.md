...
2026-06-01 在 preview 合入后发现 Qwen3.5 MTP 输出出现严重重复，例如同一道选择题虽然最终答案为 D，但中间持续重复 `</think>`、`。` 和 `answer:D`。该问题不能通过简单回退处理，应先定位 PR #1536 引入的动态参数语义问题，再给出最小正确性修复。

根因：

- PR #1536 为了复用 `causal_conv1d`，把 spec verify 的 `num_accepted_tokens` 转成 host vector / `IntArrayRef`。
- `cache_indices` 也来自 `linear_state_ids` host vector。
- 在 ACL graph capture 下，host `IntArrayRef` 会作为 capture 时的固定属性进入 executor；replay 时虽然 `persistent_num_accepted_tokens` 和 `persistent_linear_state_indices` 设备 tensor 已更新，但 conv 算子仍可能使用旧 host 值。
- `num_accepted_tokens` 影响 speculative conv state offset，`cache_indices` 决定读写哪一行 conv cache。任一值 stale 都会导致 GatedDeltaNet conv state 串行污染，进而出现重复输出和精度异常。

已验证但拒绝的方向：

- 曾尝试新增/使用 `aclnnCausalConv1dTensorGetWorkspaceSize`，把 spec verify 的 `query_start_loc/cache_indices/num_accepted_tokens` 统一作为 tensor optional input 传给 `aclnnCausalConv1d`。
- 该方向在 Qwen3.5 MTP spec decode 下不可作为当前修复：graph on 和 graph off 都会在首个 MTP graph capture/执行附近段错误。
- 当时已确认输入 shape/dtype 合法：`x=[1,4,2560]` BF16、`weight=[4,2560]` BF16、`conv_state=[89,6,2560]` BF16、`query_start_loc=[2]` int64、`cache_indices=[1]` int64、`num_accepted_tokens=[1]` int64。因此该问题不是简单的 ACL graph replay stale，而是当前 tensor workspace 路径本身不能覆盖该 spec decode 组合。

最终正确性修复方向：

- 删除 `num_accepted_tokens_vec` 这条 host 参数链，避免后续误用。
- spec verify conv 不再把 replay 会变化的参数转成 host `IntArrayRef`；`linear_state_indices`、`q_cu_seq_lens`、`num_accepted_tokens` 必须保持为 graph persistent device tensor。
- 在 tensor 版 `aclnnCausalConv1d` 可用前，Qwen3.5 MTP spec verify 回到 `causal_conv1d_update_v2` 的 tensor 动态参数路径，优先保证正确性。
- `causal_conv1d_update_v2` wrapper 期望 `conv_state` layout 为 `[slot, dim, state_len]`，而 Qwen3.5 conv cache 是 `[slot, state_len, dim]`；调用时传 `conv_cache.transpose(1, 2)`，让 wrapper 内部 copy-back 更新原始 cache。
- 该修复牺牲了 PR #1536 原始 transpose 消除优化收益，不能继续把 PR #1536 的性能表作为修复后的收益证明；后续需要新的 graph-safe/fused spec causal conv tensor API 才能同时保留性能与正确性。

验证门禁：

- 至少编译关键对象：`npu_causal_conv1d.cpp.o`、`qwen3_gated_delta_net_base.cpp.o`、`ops_api.cpp.o`。
- 精度必须覆盖 graph on、MTP=3、parallel>1、多请求 batch shape 变化；只测 parallel=1 不足以证明并发 cache index 正确。
- 性能对比前必须确认同一 preview 基线、同一 submodule、同一 CANN op 包状态，避免 worktree 间安装 `xllm_ops` 到 CANN 目录互相污染。

本轮正确性 smoke：

- 配置：TP=4、MTP=3、graph on、chunk prefill on、Qwen35-27B + Qwen35-27B-mtp。
- Prompt：UDP 可靠传输 CEval 单题，`max_tokens=4096`。
- 结果：最终稳定输出 `answer:D`，未复现重复 `</think>`、重复 `。`、重复 `answer:D` 或空回复。
- 构建：清理后完整 `cmake --build build/cmake.linux-aarch64-cpython-311 --target xllm -j8` 通过；但这是一次过宽的完整重编，后续小范围 C++ 验证应优先编译对象级目标，避免误把构建缓存 churn 当成源码问题。
