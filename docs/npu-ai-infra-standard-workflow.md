...
新引入且职责容易混淆的执行器类 skill 可以改为 `*-runner`，例如
`xllm-npu-eval-runner`，用于和 benchmark/profiler/accuracy-debug 等分析类
skill 区分。
...
- 新增通用参考文件：`references/framework-adapter-contract.md`、
  `references/run-manifest-template.md`、`references/perf-artifact-schema.md`、
  `references/accuracy-artifact-schema.md`、`references/profiling-artifact-schema.md`。
- 新增框架适配目录：
