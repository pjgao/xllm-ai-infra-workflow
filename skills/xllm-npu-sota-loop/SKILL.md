...
1. 拉取 PR 分支后，构建前先完成环境门禁：
   ...
   - 注意 `third_party/torch_npu_ops/triton_npu/binary` 是 Triton NPU 运行时必需资产，可能表现为子模块未跟踪目录；不要把它当普通脏文件清理。若缺失，先按该子模块 README 运行 `python setup.py` 生成，再回到根目录执行 xLLM 构建。
...
Qwen3.5-27B MTP 的历史 RLCR 案例见
[`references/qwen35-mtp-case.md`](references/qwen35-mtp-case.md)。只有需要具体
案例参考时再加载，常规 SOTA loop 按上面的 phase 规则执行即可。
