...
当你拿到一个新模型（如 Qwen3.5-27B）需要在昇腾 910B3/A3 或 910B/A2 上做到最优性能时，
...
1. **确定模型与硬件**  
   `export RUN_ROOT=<run-root>/$(date +%Y%m%d)_<model>_npu_sota`
...
**Q: 我可以在 A2 / 910B 上用吗？**
A: 可以。kernel-pilot 已补充 A2 / 910B 参考规格，但 tile size、block 数、
UB 约束和性能结论仍要以当前机器 profiling 为准，不要直接复用 A3 结论。
